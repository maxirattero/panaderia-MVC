// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using Panaderia.Models.Data;

#nullable disable

namespace Panaderia.Models.Migrations
{
    [DbContext(typeof(PanaderiaContext))]
    [Migration("20260924164940_AgregarDisponibilidadSemanalProducto")]
    partial class AgregarDisponibilidadSemanalProducto
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.11")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("Microsoft.AspNetCore.DataProtection.EntityFrameworkCore.DataProtectionKey", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("FriendlyName")
                        .HasColumnType("text");

                    b.Property<string>("Xml")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("DataProtectionKeys");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("text");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex");

                    b.ToTable("AspNetRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("text");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("text");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("text");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("text");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)");

                    b.Property<string>("ProviderKey")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("text");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<string>("RoleId")
                        .HasColumnType("text");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<string>("LoginProvider")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)");

                    b.Property<string>("Name")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)");

                    b.Property<string>("Value")
                        .HasColumnType("text");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens", (string)null);
                });

            modelBuilder.Entity("Panaderia.Models.Entities.ApplicationUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("text");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("integer");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("text");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("boolean");

                    b.Property<int?>("IdCliente")
                        .HasColumnType("integer");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("boolean");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("NombreCompleto")
                        .HasColumnType("text");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("text");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("text");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("boolean");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("text");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("boolean");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("IdCliente")
                        .IsUnique();

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex");

                    b.ToTable("AspNetUsers", (string)null);
                });

            modelBuilder.Entity("Panaderia.Models.Entities.CategoriaProducto", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("FechaCreacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("FechaModificacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("CategoriasProducto");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.CierreCaja", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<decimal>("Ani")
                        .HasColumnType("numeric");

                    b.Property<decimal>("BaseReparto")
                        .HasColumnType("numeric");

                    b.Property<decimal>("CobrosBanco")
                        .HasColumnType("numeric");

                    b.Property<decimal>("CobrosEfectivo")
                        .HasColumnType("numeric");

                    b.Property<decimal>("CobrosSinClasificar")
                        .HasColumnType("numeric");

                    b.Property<decimal?>("CostoInsumos")
                        .HasColumnType("numeric");

                    b.Property<decimal>("Devoluciones")
                        .HasColumnType("numeric");

                    b.Property<bool>("EsHistorico")
                        .HasColumnType("boolean");

                    b.Property<DateTime>("FinUtc")
                        .HasColumnType("timestamp with time zone");

                    b.Property<decimal>("GastosReparto")
                        .HasColumnType("numeric");

                    b.Property<int?>("IdMovimientoHistorico")
                        .HasColumnType("integer");

                    b.Property<DateOnly>("InicioSemana")
                        .HasColumnType("date");

                    b.Property<DateTime>("InicioUtc")
                        .HasColumnType("timestamp with time zone");

                    b.Property<decimal>("Maxi")
                        .HasColumnType("numeric");

                    b.Property<string>("Notas")
                        .HasColumnType("text");

                    b.Property<decimal?>("PorcentajeReserva")
                        .HasPrecision(5, 2)
                        .HasColumnType("numeric(5,2)");

                    b.Property<DateTime>("RegistradoUtc")
                        .HasColumnType("timestamp with time zone");

                    b.Property<decimal>("Reserva")
                        .HasColumnType("numeric");

                    b.Property<decimal?>("RetiroHistorico")
                        .HasColumnType("numeric");

                    b.Property<decimal?>("TotalVendido")
                        .HasColumnType("numeric");

                    b.Property<string>("Usuario")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("IdMovimientoHistorico")
                        .IsUnique();

                    b.HasIndex("InicioSemana")
                        .IsUnique()
                        .HasFilter("NOT \"EsHistorico\"");

                    b.ToTable("CierresCaja");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.CierreCajaCosto", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("Cantidad")
                        .HasColumnType("integer");

                    b.Property<bool>("CostoPendiente")
                        .HasColumnType("boolean");

                    b.Property<decimal>("Empaque")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<int>("IdCierre")
                        .HasColumnType("integer");

                    b.Property<int>("IdProducto")
                        .HasColumnType("integer");

                    b.Property<decimal>("Ingredientes")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<string>("Producto")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<bool>("Reconstruido")
                        .HasColumnType("boolean");

                    b.HasKey("Id");

                    b.HasIndex("IdCierre");

                    b.ToTable("CierresCajaCostos");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.CierreCajaSaldo", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("Cuenta")
                        .HasColumnType("integer");

                    b.Property<decimal>("Entradas")
                        .HasColumnType("numeric");

                    b.Property<decimal>("Final")
                        .HasColumnType("numeric");

                    b.Property<int>("IdCierre")
                        .HasColumnType("integer");

                    b.Property<decimal>("Inicial")
                        .HasColumnType("numeric");

                    b.Property<decimal>("Salidas")
                        .HasColumnType("numeric");

                    b.HasKey("Id");

                    b.HasIndex("IdCierre");

                    b.ToTable("CierresCajaSaldos");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Cliente", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Apellido")
                        .HasColumnType("text");

                    b.Property<decimal?>("DescuentoPorcentaje")
                        .HasPrecision(5, 2)
                        .HasColumnType("numeric(5,2)");

                    b.Property<string>("Direccion")
                        .HasColumnType("text");

                    b.Property<DateTime>("FechaCreacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("FechaModificacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Localidad")
                        .HasColumnType("text");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<bool>("PrecioDeCosto")
                        .HasColumnType("boolean");

                    b.Property<string>("Provincia")
                        .HasColumnType("text");

                    b.Property<bool>("Revendedor")
                        .HasColumnType("boolean");

                    b.Property<string>("Telefono")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Clientes");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.CompraDetalle", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<decimal>("Cantidad")
                        .HasColumnType("numeric");

                    b.Property<decimal>("CostoEnvio")
                        .HasColumnType("numeric");

                    b.Property<int>("IdCompra")
                        .HasColumnType("integer");

                    b.Property<int>("IdInsumo")
                        .HasColumnType("integer");

                    b.Property<int>("IdUnidadCompra")
                        .HasColumnType("integer");

                    b.Property<decimal>("PrecioUnitario")
                        .HasColumnType("numeric");

                    b.Property<decimal>("Subtotal")
                        .HasColumnType("numeric");

                    b.HasKey("Id");

                    b.HasIndex("IdCompra");

                    b.HasIndex("IdInsumo");

                    b.HasIndex("IdUnidadCompra");

                    b.ToTable("ComprasDetalle");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.CompraProveedor", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("Fecha")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("IdProveedor")
                        .HasColumnType("integer");

                    b.Property<decimal>("MontoTotal")
                        .HasColumnType("numeric");

                    b.Property<string>("Notas")
                        .HasColumnType("text");

                    b.Property<string>("NumeroFactura")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("IdProveedor");

                    b.ToTable("ComprasProveedor");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.ConfiguracionCaja", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer");

                    b.Property<decimal>("AperturaBanco")
                        .HasColumnType("numeric");

                    b.Property<decimal>("AperturaEfectivo")
                        .HasColumnType("numeric");

                    b.Property<decimal>("AperturaReserva")
                        .HasColumnType("numeric");

                    b.Property<DateTime?>("FechaConfiguracion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("InicioSaldosUtc")
                        .HasColumnType("timestamp with time zone");

                    b.Property<decimal>("PorcentajeReserva")
                        .HasPrecision(5, 2)
                        .HasColumnType("numeric(5,2)");

                    b.Property<string>("UsuarioConfiguracion")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("ConfiguracionCaja", null, t =>
                        {
                            t.HasCheckConstraint("CK_ConfiguracionCaja_Id", "\"Id\" = 1");

                            t.HasCheckConstraint("CK_ConfiguracionCaja_Porcentaje", "\"PorcentajeReserva\" BETWEEN 0 AND 100");
                        });

                    b.HasData(
                        new
                        {
                            Id = 1,
                            AperturaBanco = 0m,
                            AperturaEfectivo = 0m,
                            AperturaReserva = 0m,
                            PorcentajeReserva = 30m
                        });
                });

            modelBuilder.Entity("Panaderia.Models.Entities.ConfiguracionTienda", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer");

                    b.Property<decimal>("MontoMinimoPedido")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)");

                    b.Property<bool>("RetiroHabilitado")
                        .HasColumnType("boolean");

                    b.HasKey("Id");

                    b.ToTable("ConfiguracionTienda", null, t =>
                        {
                            t.HasCheckConstraint("CK_ConfiguracionTienda_Minimo", "\"MontoMinimoPedido\" >= 0");

                            t.HasCheckConstraint("CK_ConfiguracionTienda_Unica", "\"Id\" = 1");
                        });

                    b.HasData(
                        new
                        {
                            Id = 1,
                            MontoMinimoPedido = 0m,
                            RetiroHabilitado = true
                        });
                });

            modelBuilder.Entity("Panaderia.Models.Entities.DetallePedido", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("Cantidad")
                        .HasColumnType("integer");

                    b.Property<int>("CantidadProducida")
                        .HasColumnType("integer");

                    b.Property<decimal>("CostoEmpaque")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<decimal?>("CostoIngredientes")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<DateTime?>("FechaCosto")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int?>("IdEmpaque")
                        .HasColumnType("integer");

                    b.Property<int>("IdPedido")
                        .HasColumnType("integer");

                    b.Property<int>("IdProducto")
                        .HasColumnType("integer");

                    b.Property<bool>("LlevaEtiqueta")
                        .HasColumnType("boolean");

                    b.Property<decimal>("PrecioUnitario")
                        .HasColumnType("numeric");

                    b.Property<bool>("ReservaStock")
                        .HasColumnType("boolean");

                    b.HasKey("Id");

                    b.HasIndex("IdEmpaque");

                    b.HasIndex("IdPedido");

                    b.HasIndex("IdProducto");

                    b.ToTable("DetallesPedido");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Etiqueta", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("FechaCreacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("FechaModificacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Icono")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)");

                    b.HasKey("Id");

                    b.HasIndex("Nombre")
                        .IsUnique();

                    b.ToTable("Etiquetas");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Formato", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Descripcion")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Formatos");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Insumo", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<bool>("Activo")
                        .HasColumnType("boolean");

                    b.Property<decimal>("CantidadRendimiento")
                        .HasColumnType("numeric");

                    b.Property<bool>("EsBolsaPapel")
                        .HasColumnType("boolean");

                    b.Property<DateTime>("FechaCreacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("FechaModificacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int?>("IdProveedor")
                        .HasColumnType("integer");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Notas")
                        .HasColumnType("text");

                    b.Property<decimal>("PrecioCompra")
                        .HasColumnType("numeric");

                    b.Property<decimal>("StockActual")
                        .HasColumnType("numeric");

                    b.Property<decimal?>("StockMinimo")
                        .HasColumnType("numeric");

                    b.Property<int>("TipoInsumo")
                        .HasColumnType("integer");

                    b.Property<int>("UnidadBase")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("IdProveedor");

                    b.ToTable("Insumos");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Pedido", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<bool>("Anulado")
                        .HasColumnType("boolean");

                    b.Property<decimal>("DescuentoPorcentaje")
                        .HasPrecision(5, 2)
                        .HasColumnType("numeric(5,2)");

                    b.Property<int>("Estado")
                        .HasColumnType("integer");

                    b.Property<DateTime>("FechaCreacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("FechaEntrega")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("FechaEntregaReal")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("FechaModificacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("IdCliente")
                        .HasColumnType("integer");

                    b.Property<decimal>("MontoCobrado")
                        .HasColumnType("numeric");

                    b.Property<decimal>("MontoTotal")
                        .HasColumnType("numeric");

                    b.Property<string>("Notas")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("IdCliente");

                    b.HasIndex("FechaEntregaReal", "Estado");

                    b.ToTable("Pedidos");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.ProduccionStock", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("Cantidad")
                        .HasColumnType("integer");

                    b.Property<DateTime>("Fecha")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("IdProducto")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("IdProducto");

                    b.ToTable("ProduccionStock");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Producto", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("DescripcionTienda")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<DateTime>("FechaCreacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("FechaModificacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("IdCategoria")
                        .HasColumnType("integer");

                    b.Property<int?>("IdFormato")
                        .HasColumnType("integer");

                    b.Property<int?>("IdTamano")
                        .HasColumnType("integer");

                    b.Property<string>("ImagenURL")
                        .HasColumnType("text");

                    b.Property<string>("Ingredientes")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<int?>("Masa")
                        .HasColumnType("integer");

                    b.Property<string>("Nombre")
                        .HasColumnType("text");

                    b.Property<string>("ObservacionesElaboracion")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<bool>("OcultoEnTienda")
                        .HasColumnType("boolean");

                    b.Property<bool>("PorEncargo")
                        .HasColumnType("boolean");

                    b.Property<decimal>("PrecioFinal")
                        .HasColumnType("numeric");

                    b.Property<decimal>("PrecioReventa")
                        .HasColumnType("numeric");

                    b.Property<bool>("SinStock")
                        .HasColumnType("boolean");

                    b.Property<int>("Stock")
                        .HasColumnType("integer");

                    b.Property<bool>("TieneDisponibilidadSemanal")
                        .HasColumnType("boolean");

                    b.Property<int?>("Variedad")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("IdCategoria");

                    b.HasIndex("IdFormato");

                    b.HasIndex("IdTamano");

                    b.ToTable("Productos");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.ProductoEtiqueta", b =>
                {
                    b.Property<int>("IdProducto")
                        .HasColumnType("integer");

                    b.Property<int>("IdEtiqueta")
                        .HasColumnType("integer");

                    b.HasKey("IdProducto", "IdEtiqueta");

                    b.HasIndex("IdEtiqueta");

                    b.ToTable("ProductoEtiquetas");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.ProductoImagen", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("ContentType")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<byte[]>("Datos")
                        .IsRequired()
                        .HasColumnType("bytea");

                    b.Property<DateTime>("FechaCreacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("FechaModificacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("IdProducto")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("IdProducto")
                        .IsUnique();

                    b.ToTable("ProductoImagenes");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Proveedor", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<bool>("Activo")
                        .HasColumnType("boolean");

                    b.Property<string>("Contacto")
                        .HasColumnType("text");

                    b.Property<string>("Direccion")
                        .HasColumnType("text");

                    b.Property<string>("Email")
                        .HasColumnType("text");

                    b.Property<DateTime>("FechaCreacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("FechaModificacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Notas")
                        .HasColumnType("text");

                    b.Property<string>("Telefono")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Proveedores");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Receta", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("FechaCreacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("FechaModificacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("IdProducto")
                        .HasColumnType("integer");

                    b.Property<decimal>("PesoUnitario")
                        .HasColumnType("numeric");

                    b.Property<int>("TamanioLote")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("IdProducto")
                        .IsUnique();

                    b.ToTable("Recetas");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.RecetaDetalle", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<decimal?>("CantidadFija")
                        .HasColumnType("numeric");

                    b.Property<int?>("IdInsumo")
                        .HasColumnType("integer");

                    b.Property<int>("IdReceta")
                        .HasColumnType("integer");

                    b.Property<int?>("IdSubReceta")
                        .HasColumnType("integer");

                    b.Property<decimal?>("PorcentajePanadero")
                        .HasColumnType("numeric");

                    b.HasKey("Id");

                    b.HasIndex("IdInsumo");

                    b.HasIndex("IdReceta");

                    b.HasIndex("IdSubReceta");

                    b.ToTable("RecetaDetalles");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.ReporteCaja", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("Categoria")
                        .HasColumnType("integer");

                    b.Property<Guid?>("ClaveOperacion")
                        .HasColumnType("uuid");

                    b.Property<int>("Cuenta")
                        .HasColumnType("integer");

                    b.Property<bool>("DescontarDelReparto")
                        .HasColumnType("boolean");

                    b.Property<string>("Descripcion")
                        .HasColumnType("text");

                    b.Property<int?>("Destino")
                        .HasColumnType("integer");

                    b.Property<DateTime>("Fecha")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("FechaFinPeriodo")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("FechaInicioPeriodo")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int?>("IdCierre")
                        .HasColumnType("integer");

                    b.Property<int?>("IdCierreDestino")
                        .HasColumnType("integer");

                    b.Property<int?>("IdCompra")
                        .HasColumnType("integer");

                    b.Property<int?>("IdPedido")
                        .HasColumnType("integer");

                    b.Property<int?>("IdProveedor")
                        .HasColumnType("integer");

                    b.Property<Guid?>("IdTransferencia")
                        .HasColumnType("uuid");

                    b.Property<decimal>("Monto")
                        .HasColumnType("numeric");

                    b.Property<int>("Tipo")
                        .HasColumnType("integer");

                    b.Property<decimal?>("TotalVendidoInformativo")
                        .HasColumnType("numeric");

                    b.HasKey("Id");

                    b.HasIndex("ClaveOperacion")
                        .IsUnique();

                    b.HasIndex("Fecha");

                    b.HasIndex("IdCierre");

                    b.HasIndex("IdCierreDestino");

                    b.HasIndex("IdCompra");

                    b.HasIndex("IdPedido");

                    b.HasIndex("IdProveedor");

                    b.HasIndex("Cuenta", "Fecha");

                    b.HasIndex("IdTransferencia", "Tipo")
                        .IsUnique();

                    b.ToTable("ReportesCaja");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.SubReceta", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<decimal>("MargenSeguridad")
                        .HasColumnType("numeric");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Notas")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("SubRecetas");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.SubRecetaDetalle", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<decimal?>("CantidadFija")
                        .HasColumnType("numeric");

                    b.Property<int>("IdInsumo")
                        .HasColumnType("integer");

                    b.Property<int>("IdSubReceta")
                        .HasColumnType("integer");

                    b.Property<decimal?>("PorcentajePanadero")
                        .HasColumnType("numeric");

                    b.HasKey("Id");

                    b.HasIndex("IdInsumo");

                    b.HasIndex("IdSubReceta");

                    b.ToTable("SubRecetaDetalles");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.SuscripcionPush", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Auth")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)");

                    b.Property<string>("Endpoint")
                        .IsRequired()
                        .HasMaxLength(1024)
                        .HasColumnType("character varying(1024)");

                    b.Property<DateTime>("FechaCreacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("FechaModificacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("P256dh")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasMaxLength(450)
                        .HasColumnType("character varying(450)");

                    b.HasKey("Id");

                    b.HasIndex("Endpoint")
                        .IsUnique();

                    b.HasIndex("UserId");

                    b.ToTable("SuscripcionesPush");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Tamano", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Descripcion")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Tamanos");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.UnidadCompra", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<bool>("EsPorDefecto")
                        .HasColumnType("boolean");

                    b.Property<decimal>("FactorConversion")
                        .HasColumnType("numeric");

                    b.Property<int>("IdInsumo")
                        .HasColumnType("integer");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("IdInsumo");

                    b.ToTable("UnidadesCompra");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Panaderia.Models.Entities.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Panaderia.Models.Entities.ApplicationUser", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.Cliente", null)
                        .WithMany()
                        .HasForeignKey("IdCliente")
                        .OnDelete(DeleteBehavior.Restrict);
                });

            modelBuilder.Entity("Panaderia.Models.Entities.CierreCajaCosto", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.CierreCaja", "Cierre")
                        .WithMany("Costos")
                        .HasForeignKey("IdCierre")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Cierre");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.CierreCajaSaldo", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.CierreCaja", "Cierre")
                        .WithMany("Saldos")
                        .HasForeignKey("IdCierre")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Cierre");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.CompraDetalle", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.CompraProveedor", "Compra")
                        .WithMany("Detalles")
                        .HasForeignKey("IdCompra")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Panaderia.Models.Entities.Insumo", "Insumo")
                        .WithMany()
                        .HasForeignKey("IdInsumo")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Panaderia.Models.Entities.UnidadCompra", "UnidadCompra")
                        .WithMany()
                        .HasForeignKey("IdUnidadCompra")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Compra");

                    b.Navigation("Insumo");

                    b.Navigation("UnidadCompra");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.CompraProveedor", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.Proveedor", "Proveedor")
                        .WithMany()
                        .HasForeignKey("IdProveedor")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Proveedor");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.DetallePedido", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.Insumo", "Empaque")
                        .WithMany()
                        .HasForeignKey("IdEmpaque")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Panaderia.Models.Entities.Pedido", "Pedido")
                        .WithMany("Detalles")
                        .HasForeignKey("IdPedido")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Panaderia.Models.Entities.Producto", "Producto")
                        .WithMany()
                        .HasForeignKey("IdProducto")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Empaque");

                    b.Navigation("Pedido");

                    b.Navigation("Producto");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Insumo", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.Proveedor", "Proveedor")
                        .WithMany()
                        .HasForeignKey("IdProveedor")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Proveedor");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Pedido", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.Cliente", "Cliente")
                        .WithMany()
                        .HasForeignKey("IdCliente")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Cliente");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.ProduccionStock", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.Producto", "Producto")
                        .WithMany()
                        .HasForeignKey("IdProducto")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Producto");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Producto", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.CategoriaProducto", "Categoria")
                        .WithMany()
                        .HasForeignKey("IdCategoria")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Panaderia.Models.Entities.Formato", "Formato")
                        .WithMany()
                        .HasForeignKey("IdFormato");

                    b.HasOne("Panaderia.Models.Entities.Tamano", "Tamano")
                        .WithMany()
                        .HasForeignKey("IdTamano");

                    b.Navigation("Categoria");

                    b.Navigation("Formato");

                    b.Navigation("Tamano");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.ProductoEtiqueta", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.Etiqueta", "Etiqueta")
                        .WithMany()
                        .HasForeignKey("IdEtiqueta")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Panaderia.Models.Entities.Producto", "Producto")
                        .WithMany("ProductoEtiquetas")
                        .HasForeignKey("IdProducto")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Etiqueta");

                    b.Navigation("Producto");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.ProductoImagen", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.Producto", "Producto")
                        .WithMany()
                        .HasForeignKey("IdProducto")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Producto");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Receta", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.Producto", "Producto")
                        .WithMany()
                        .HasForeignKey("IdProducto")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Producto");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.RecetaDetalle", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.Insumo", "Insumo")
                        .WithMany()
                        .HasForeignKey("IdInsumo")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Panaderia.Models.Entities.Receta", "Receta")
                        .WithMany("Detalles")
                        .HasForeignKey("IdReceta")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Panaderia.Models.Entities.SubReceta", "SubReceta")
                        .WithMany()
                        .HasForeignKey("IdSubReceta")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Insumo");

                    b.Navigation("Receta");

                    b.Navigation("SubReceta");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.ReporteCaja", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.CierreCaja", "Cierre")
                        .WithMany()
                        .HasForeignKey("IdCierre")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Panaderia.Models.Entities.CierreCaja", "CierreDestino")
                        .WithMany()
                        .HasForeignKey("IdCierreDestino")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Panaderia.Models.Entities.CompraProveedor", "Compra")
                        .WithMany()
                        .HasForeignKey("IdCompra")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Panaderia.Models.Entities.Pedido", "Pedido")
                        .WithMany("Reportes")
                        .HasForeignKey("IdPedido");

                    b.HasOne("Panaderia.Models.Entities.Proveedor", "Proveedor")
                        .WithMany()
                        .HasForeignKey("IdProveedor");

                    b.Navigation("Cierre");

                    b.Navigation("CierreDestino");

                    b.Navigation("Compra");

                    b.Navigation("Pedido");

                    b.Navigation("Proveedor");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.SubRecetaDetalle", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.Insumo", "Insumo")
                        .WithMany()
                        .HasForeignKey("IdInsumo")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Panaderia.Models.Entities.SubReceta", "SubReceta")
                        .WithMany("Detalles")
                        .HasForeignKey("IdSubReceta")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Insumo");

                    b.Navigation("SubReceta");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.SuscripcionPush", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.ApplicationUser", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.UnidadCompra", b =>
                {
                    b.HasOne("Panaderia.Models.Entities.Insumo", "Insumo")
                        .WithMany("UnidadesCompra")
                        .HasForeignKey("IdInsumo")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Insumo");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.CierreCaja", b =>
                {
                    b.Navigation("Costos");

                    b.Navigation("Saldos");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.CompraProveedor", b =>
                {
                    b.Navigation("Detalles");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Insumo", b =>
                {
                    b.Navigation("UnidadesCompra");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Pedido", b =>
                {
                    b.Navigation("Detalles");

                    b.Navigation("Reportes");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Producto", b =>
                {
                    b.Navigation("ProductoEtiquetas");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.Receta", b =>
                {
                    b.Navigation("Detalles");
                });

            modelBuilder.Entity("Panaderia.Models.Entities.SubReceta", b =>
                {
                    b.Navigation("Detalles");
                });
#pragma warning restore 612, 618
        }
    }
}
